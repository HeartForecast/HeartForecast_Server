...
public class CustomLogoutFilter extends GenericFilterBean {
    ...
    private void doFilter(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws IOException, ServletException {
        ...
        if (refreshNormal != null) processLogout(refreshNormal, "refresh_normal", response);
        else if (refreshSocial != null) processLogout(refreshSocial, "refresh_social", response);
        else throw new RefreshTokenNotFoundException();
    }

    private void processLogout(String refreshToken, String cookieName, HttpServletResponse response) {

        String category = jwtUtil.getCategory(refreshToken);
        if (!category.equals("refresh")) throw new InvalidTokenException();

        Boolean isExist = refreshRepository.existsByRefreshToken(refreshToken);
        if (!isExist) throw new RefreshTokenNotFoundException();
